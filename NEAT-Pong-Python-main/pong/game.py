...
class Game:
    ...
    def __init__(self, window, window_width, window_height):
        ...
        self.left_winner = "Left Player Won!"
        self.right_winner = "Right Player Won!"
    ...
    def _draw_winner(self):
        left_winner_text = self.WINNER_FONT.render(
            f"{self.left_winner}", 1, self.WHITE)
        right_winner_text = self.WINNER_FONT.render(
            f"{self.right_winner}", 1, self.WHITE)
        if self.left_score >= 5:
            self.window.blit(left_winner_text, (self.window_width // 4 - left_winner_text.get_width()//2,
                                                self.window_height // 4 - left_winner_text.get_height() // 2))
        if self.right_score >= 5:
            self.window.blit(right_winner_text, (self.window_width * (3/4) - right_winner_text.get_width()//2,
                                                 self.window_height // 4 - left_winner_text.get_height() // 2))
